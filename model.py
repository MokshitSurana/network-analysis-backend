...
class BertForTokenAndSequenceJointClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        gates = gate.unsqueeze(1).repeat(
            1, token_logits.size()[1], token_logits.size()[2]
        )
        ...
        if labels is not None:
            ...
            weighted_token_logits = weighted_token_logits.view(
                -1, weighted_token_logits.shape[-1]
            )
            sequence_logits = sequence_logits.view(-1, sequence_logits.shape[-1])

            token_loss = criterion(weighted_token_logits, labels)
            sequence_label = torch.LongTensor(
                [1] if any(label > 0 for label in labels) else [0]
            )
